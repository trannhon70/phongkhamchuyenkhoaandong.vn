    ...
    if (!empty($sdt)  ) {
        ...
        if ($check_result && $check_result->num_rows > 0) {
            ...
            $sql = "INSERT INTO admin_khachhang (hoten, ngaysinh, sdt, trieuchung,ngaykham,giokham, status, note, ketqua, nguon, user_tuvan, created_at,form) 
                VALUES ('$hoten', '', '$sdt', '','','$created_at', 0, '', 0, '$nguon', 0, '$created_at', '')";

            $result = $db->insert($sql);

            if ($result) {
                echo json_encode(['status' => 'success', 'message' => 'Cảm ơn bạn đã để lại thông tin, phòng khám sẽ liên hệ với bạn trong thời gian sớm nhất!']);
            } else {
